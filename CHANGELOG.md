...
- **MCP Quality Engine v1.1 (trust & coverage hardening)**: the quality score is now protocol-version aware, fully derived from visible diagnostics (no hidden deductions), and honest about what it actually inspected.
  ...
  - `quality.coverage`/`coveragePercent` (new, on `RunReport.quality`): tells you which of the 5 dimensions were actually evaluated by the checks that ran, as `'covered' | 'partial' | 'not-covered'` — a security score of 100 from a schema-only check run now correctly shows `security: 'not-covered'` instead of implying a full audit.
  - `quality.scoredServers`/`unscoredServers` (new): a server that failed to connect is never silently dropped from a fleet's aggregate score — it's named explicitly.
  - `quality.disclaimer` (new) and a `Coverage: N%` line in the human report: "this score reflects issues detectable by mcp-medic's passive inspection — it is not a certification."
  - `quality.minimumScore` policy now also warns (`policy.partial-coverage-with-minimum-score`) when the gate was evaluated against incomplete coverage or an unscored server, so a passing minimum-score check can't be silently misleading.
